...
void WifiConfigurationAp::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        ESP_LOGE("wifi", "disconnected: reason=%d,  count=%d",(int)e->reason, disconnect_count);
        ...
        {
            esp_timer_start_once(self->scan_timer_, 1 * 1000000); // 1秒后再次扫描
        }
        // // 扫描完成，等待10秒后再次扫描
...
void WifiConfigurationAp::IpEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == IP_EVENT_STA_GOT_IP) {
        ...
        if(doit_blufi_is_init()) {
            esp_blufi_send_wifi_conn_report(mode, ESP_BLUFI_STA_CONN_SUCCESS, 1, NULL);
...
std::vector<wifi_ap_record_t> WifiConfigurationAp::GetScanResults() {
    std::lock_guard<std::mutex> lock(mutex_);
    return ap_records_; // 直接返回副本，线程安全
}
