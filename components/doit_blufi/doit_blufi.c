...
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    ...
    case ESP_BLUFI_EVENT_RECV_CUSTOM_DATA:
        ...
        if (param->custom_data.data_len > 7) { // 至少需要"AT+OTA="的长度
          ...
          if (data_str) {
            memcpy(data_str, param->custom_data.data,
                   param->custom_data.data_len);
            data_str[param->custom_data.data_len] =
                '\0'; // 确保字符串以null结尾
            // 检查是否是AT+OTA命令
...
bool doit_blufi_send_code(uint8_t *code){
    ...
    esp_err_t err = esp_blufi_send_custom_data((uint8_t *)code, 6);
    if(err != ESP_OK){
        ESP_LOGE(TAG, "BLUFI send code failed: %s", esp_err_to_name(err));
        return false;
    }
    ESP_LOGI(TAG, "BLUFI send code success");
    return true;
}
