...
private:
    ...
    std::function<void(bool)> on_charging_status_changed_ = nullptr;
    std::function<void(bool)> on_low_battery_status_changed_ = nullptr;
    std::function<void(uint32_t)> on_battery_level_changed_ = nullptr;
    ...
    bool IsReservedFlashGpio(gpio_num_t gpio) {
#if (defined(CONFIG_IDF_TARGET_ESP32C3) || defined(CONFIG_IDF_TARGET_ESP32C2))
        return (gpio >= GPIO_NUM_6 && gpio <= GPIO_NUM_11);
#else
        (void)gpio;
        return false;
#endif
...
public:
    PowerManager(gpio_num_t charging_pin, gpio_num_t adc_pin, int voltage_max, int voltage_min, adc_oneshot_unit_handle_t adc_handle = nullptr) : charging_pin_(charging_pin), voltage_max_(voltage_max), voltage_min_(voltage_min), adc_handle_(adc_handle){
        ...
        // 初始化充电引脚
        if (IsReservedFlashGpio(charging_pin_)) {
            ESP_LOGW(TAG, "charging gpio %d is reserved for flash, ignore", charging_pin_);
    ...
    void OnChargingLevelChanged(std::function<void(uint32_t)> callback){
