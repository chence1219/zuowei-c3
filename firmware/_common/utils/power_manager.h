...
private:
    ...
    gpio_num_t power_keep_pin_ = GPIO_NUM_NC;
    int power_keep_enable_level_ = 1;
    std::vector<uint16_t> adc_values_;
    uint32_t battery_level_ = 100;
    bool is_charging_ = false;
    bool is_low_battery_ = false;
    int ticks_ = 0;
    int power_off_battery_level_ticks_ = 0; // 达到关机电量，关机计时
    const int kBatteryAdcInterval = 60;
    const int kBatteryAdcDataCount = 3;
    const int kLowBatteryLevel = 20;
    const int kPowerOffBatteryLevel = 2; // 关机电量
    const int kPowerOffBatteryLevelMaxTicks = 10; // 达到关机电量，在这个秒数内没有充电则关机
    ...
    void CheckBatteryStatus() {
      ...
      // 检查是否需要关机
      CheckPowerOffCondition();
    }

    void CheckPowerOffCondition() {
      // 如果电量低于关机电量阈值
      if (battery_level_ <= kPowerOffBatteryLevel) {
        // 如果正在充电，则重置关机计时
        if (is_charging_) {
          power_off_battery_level_ticks_ = 0;
        } else {
          // 如果没有充电，则增加计时
          power_off_battery_level_ticks_++;

          // 如果达到最大关机等待时间，则执行关机
          if (power_off_battery_level_ticks_ >= kPowerOffBatteryLevelMaxTicks) {
            PowerOff();
          }
        }
      } else {
    ...
    void PowerOff() {
      ESP_LOGW(
          TAG,
          "Battery level is too low and not charging, system will power off");
      // 执行关机操作
      if (power_keep_pin_ != GPIO_NUM_NC) {
        Application::GetInstance().PlaySound(Lang::Sounds::OGG_POWER_OFF);
        gpio_set_level(power_keep_pin_, !power_keep_enable_level_);
...
public:
    PowerManager(gpio_num_t charging_pin, gpio_num_t adc_pin, int voltage_max, int voltage_min, adc_oneshot_unit_handle_t adc_handle = nullptr, gpio_num_t power_keep_pin = GPIO_NUM_NC, int power_keep_enable_level = 1) : charging_pin_(charging_pin), power_keep_pin_(power_keep_pin), power_keep_enable_level_(power_keep_enable_level), voltage_max_(voltage_max), voltage_min_(voltage_min), adc_handle_(adc_handle) {
