...
void Application::CheckNewVersion(Ota& ota) {
    ...
    while (true) {
        ...
        if (ota.HasNewVersion()){
            ...
            std::string message = std::string(Lang::Strings::NEW_VERSION) + _ota->GetFirmwareVersion();
            ...
            _ota->StartUpgrade([display](int progress, size_t speed) {
                char buffer[64];
...
void Application::CheckNewVersionForCustom() {
    ...
    if (custom_ota.HasNewVersion()) {
        ...
        custom_ota.StartUpgrade([display](int progress, size_t speed) {
            char buffer[64];
